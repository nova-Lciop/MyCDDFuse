...
class PerceptualLoss(torch.nn.Module):#感知损失
...
class ContrastiveLoss(torch.nn.Module):#对比损失
    def __init__(self, vgg19_weights=None):
...
class ContrastiveLoss_multiNegative(torch.nn.Module):#对比损失
    """
    ...
    def __init__(self, vgg19_weights=None):
        ...
        self.vgg = Vgg19(vgg19_weights=vgg19_weights)
        self.criterion = nn.L1Loss()#计算绝对误差损失
        self.weights = [1.0/32, 1.0/16, 1.0/8, 1.0/4, 1.0]
