...
class BaseFeatureExtraction(nn.Module):#Lite Transformer，Encode中的base
    def __init__(self,
                 dim,
                 num_heads,
...
class DetailFeatureExtraction(nn.Module): # INN 局部特征提取
    def __init__(self, num_layers=3):
        super(DetailFeatureExtraction, self).__init__()
        INNmodules = [DetailNode() for _ in range(num_layers)] # 三个DetailNode对象，一个对象包含
        self.net = nn.Sequential(*INNmodules)
    def forward(self, x):
        z1, z2 = x[:, :x.shape[1]//2], x[:, x.shape[1]//2:x.shape[1]]# 将输入张量分割成两部分
...
class LayerNorm(nn.Module):
    def __init__(self, dim, LayerNorm_type):
        ...
        if LayerNorm_type == 'BiasFree':
            self.body = BiasFree_LayerNorm(dim) #无偏置归一化
        else:
            self.body = WithBias_LayerNorm(dim) # 有偏置归一化

    def forward(self, x):
        h, w = x.shape[-2:]
        return to_4d(self.body(to_3d(x)), h, w)# 经过归一化处理后恢复到4维
...
class FeedForward(nn.Module):
    ...
    def forward(self, x):
        x = self.project_in(x)
        x1, x2 = self.dwconv(x).chunk(2, dim=1)# 深度可分离卷积
        x = F.gelu(x1) * x2
        x = self.project_out(x)
        return x
...
##########################################################################
class TransformerBlock(nn.Module): # 卷积之后的Transfrom之后的块
    def __init__(self, dim, num_heads, ffn_expansion_factor, bias, LayerNorm_type):#哈哈一模一样
...
class Restormer_Encoder(nn.Module):
    def __init__(self,
        ...
        self.patch_embed = OverlapPatchEmbed(inp_channels, dim)# 就是一个卷积

        self.encoder_level1 = nn.Sequential(*[TransformerBlock(dim=dim, num_heads=heads[0], ffn_expansion_factor=ffn_expansion_factor,
                                            bias=bias, LayerNorm_type=LayerNorm_type) for i in range(num_blocks[0])]) #串联四层TransformerBlock
