...
DIDF_Encoder = Restormer_Encoder().to(device)  # INN块在这里面，去除nn.DataParallel包装
DIDF_Decoder = Restormer_Decoder().to(device)  # INN块在这里面，去除nn.DataParallel包装
BaseFuseLayer = BaseFeatureExtraction(dim=64, num_heads=8).to(device)  # 低频特征，去除nn.DataParallel包装
DetailFuseLayer = DetailFeatureExtraction(num_layers=1).to(device)  # 高频特征，去除nn.DataParallel包装
...
for epoch in range(num_epochs):
    ...
    for i, (data_VIS, data_IR) in enumerate(loader['train']):  # 枚举红外和可见光数据
        data_VIS, data_IR = data_VIS.to(device), data_IR.to(device)  # 将数据移到指定GPU设备上
